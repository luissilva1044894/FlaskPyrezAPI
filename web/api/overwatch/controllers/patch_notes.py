...
def patch_notes_func(url, class_name):
  def get_patch_note_id(page):
    if page and hasattr(page, 'find_all'):
      for _ in page.find_all(class_=class_name) or []:#patch-notes-patch
        if isinstance(_, element.Tag) and 'div' in _.name:
          _id = _.get('id')
          if _id:
            return _id
    return ''
  def get_patch_note_title(page):
    if page and hasattr(page, 'find'):
      for patch in page.find(class_=class_name) or []:#patch-notes-patch
        if hasattr(patch, 'children'):
          title = next(patch.children, None)
          if title and not '/' in str(title):
            return str(title).strip()
    return 'Latest Overwatch Patch Notes'
  _page = get_html(url)
  return f'{get_patch_note_title(_page)} - Click here to view all the patch notes: {url.replace("pc", "")}#{get_patch_note_id(_page)}'
'''
