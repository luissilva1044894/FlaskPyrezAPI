...
if is_async():
		...
		async def wrapper(*args, **kwargs):
			...
			try:
				...
				if data and data == TOKEN:
					...
					await broadcast('New connection established', websocket)
					return await func(*args, **kwargs)
			finally:
				connected_websockets.remove(websocket._get_current_object())	
		return wrapper

	async def broadcast(msg, sender=None):
		for ws in connected_websockets:
			if (sender and not ws == sender) or not sender:
				await ws.send(msg)
	@blueprint.route('/', strict_slashes=False)
	async def http():
		"""https://github.com/pgjones/quart/blob/master/docs/websocket_tutorial.rst#5-broadcasting"""
		from quart import render_template
		return await render_template('{}/queue.html'.format(__name__.split('.')[-3]), __name__='discord')
	...
	async def consumer():
		while True:
			data = await websocket.receive()
			if data:
				await broadcast(f'echo {data}', sender=websocket)
	async def producer():
		while True:
			await asyncio.sleep(10)
			#await websocket.send('Message')
			await broadcast('Message', sender=None)
			#Desconectar
