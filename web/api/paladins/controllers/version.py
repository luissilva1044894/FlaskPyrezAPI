...
def get_server_status(_api, lang=1):
  ...
  if not _server or _server and not _server.updated:
    try:
      ...
      _patch_notes, _patch_note = get_url(f'https://cms.paladins.com/wp-json/api/get-posts/1?&search={_title[:_title.rfind("update") - 1]}'), []
      [ _.delete() for _ in PatchNote.query.all()]
      for i in range(len(_patch_notes)):
        for _lang in [1, 2, 3, 9, 10, 11, 12, 13]:
          x = get_url(f'https://cms.paladins.com/wp-json/api/get-post/{_lang}?&slug={_patch_notes[i].get("slug")}')
