...
function addCommandOutput(codeMsgChat, codeMsgBackend, botName, lang="en") {
    ...
    divMsg += "<code onclick=\"copyToClipboard()\" id=\"code-chat-bot\">{CODE_CHAT}</code></h4></div>".replace("{CODE_CHAT}", codeMsgChat);
    divMsg += "<div><button onclick=\"copyToClipboard()\" class=\"nav-link waves-effect\"  data-clipboard-target=\"code-chat-bot\" aria-label=\"Copy to clipboard\" data-original-label=\"Copy to clipboard\"><i class=\"fa fa-copy\" aria-hidden=\"true\"></i></button></div>"
