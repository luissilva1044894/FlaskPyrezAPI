...
def rank_func(battle_net, platform, paladins_like=False, format_average_sr=False):
  ...
  try:
    ...
    rank = {}
    last_time = time.time()
    r = get_html(f'https://playoverwatch.com/en-us/career/{platform}/{get_battle_net(battle_net)}')
    for item in r.findAll('div', {'class': 'competitive-rank-role'}):
  ...
  _json = get_url(f'https://ow-api.com/v1/stats/{platform}/us/{get_battle_net(battle_net)}/profile', surpress_exception=False)
